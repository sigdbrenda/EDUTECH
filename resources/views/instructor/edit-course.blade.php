        ...
        <main class="flex-1 p-10">
            <div class="max-w-4xl mx-auto">
                <div x-show="tab === 'informacion'" x-cloak class="animate-fadeIn">
                    ...
                    <form action="{{ route('courses.update', $course->id) }}" method="POST" class="space-y-8">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="bg-white p-8 rounded-xl border border-gray-200 shadow-sm" 
                            x-data="{ 
                                ...
                                seleccionadas: [],
                                opciones: {
                                    'Programación': ['Desarrollo Web', 'Back-end', 'Lenguajes', 'Algoritmos', 'Móvil'],
                                    'IA': ['Aprendizaje Profundo', 'Visión Artificial', 'NLP', 'Agentes IA', 'Robótica'],
                                    'Diseño': ['UI/UX', 'Ilustración Digital', 'Animación 2D', 'Branding', 'Diseño Web'],
                                    'Marketing': ['SEO', 'SEM', 'Redes Sociales', 'Copywriting', 'Email Marketing'],
                                    'Datos': ['Big Data', 'Visualización', 'Estadística', 'Minería de Datos', 'SQL Avanzado']
                                },
                                toggleSeleccion(item) {
                                    if (this.seleccionadas.includes(item)) {
                                        this.seleccionadas = this.seleccionadas.filter(i => i !== item);
                                ...
                                <div class="w-1/3 border-r border-gray-100 pr-4 space-y-1">
                                    <template x-for="(items, cat) in opciones">
                                        <button type="button" 
                                                ...
                                                :class="categoriaSeleccionada === cat ? 'bg-[#e6f9f1] text-[#00c471] font-bold' : 'text-gray-600 hover:bg-gray-50'"
                                            ...
                                            <span x-show="categoriaSeleccionada === cat" class="text-xs">▶</span>
                                        </button>
                                    </template>
                                </div>
                                ...
                                <div class="w-2/3 grid grid-cols-2 gap-3 content-start">
                                    <template x-for="opcion in opciones[categoriaSeleccionada]">
                                        <button type="button" 
                                                ...
                                                :class="seleccionadas.includes(opcion) ? 'border-[#00c471] bg-green-50 text-[#00c471] font-bold' : 'border-gray-200 text-gray-500 hover:border-[#00c471]'"
                                                class="border p-3 rounded-xl text-xs flex justify-between items-center transition group">
                                            <span x-text="opcion"></span>
                                            <span :class="seleccionadas.includes(opcion) ? 'bg-[#00c471] text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-[#00c471] group-hover:text-white'" 
                                                class="w-5 h-5 rounded-full flex items-center justify-center transition-colors">
                                                <span x-text="seleccionadas.includes(opcion) ? '✓' : '+' "></span>
                                            </span>
                                        </button>
                                    </template>
                                </div>
                                ...
                                <div class="space-y-3">
                                    <template x-for="(item, index) in items" :key="index">
                                        <div class="flex items-center space-x-3 group animate-fadeIn">
                                            <div class="flex-1 relative flex items-center bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                                                <span class="text-gray-300 mr-3 cursor-move">:::</span>
                                                <input type="text" name="learning_outcomes[]" x-model="items[index]" 
                                                    class="w-full border-none focus:ring-0 text-sm text-gray-600 p-0">
                                                
                                                <button type="button" @click="removeItem(index)" class="ml-auto text-red-400 hover:text-red-600 transition-colors">
